...
import DoubleArrowDown from 'packages/core/Icons/DoubleArrowDown';
...
const Pagination = (props: PaginationProps) => {
  ...
  return (
    <Container className={className}>
      {!noControls && (
        <Fragment>
          <Button isFilled onClick={setPage(1)}>
            <DoubleArrow rotate={90} />
          </Button>
          <Button isFilled onClick={prev}>
            <Arrow rotate={90} />
          </Button>
        </Fragment>
      )}
      ...
      {!noControls && (
        <Fragment>
          <Button isFilled onClick={next}>
            <Arrow rotate={-90} />
          </Button>
          <Button isFilled onClick={setPage(total)}>
            <DoubleArrow rotate={-90} />
          </Button>
        </Fragment>
      )}
...
const Button = styled.div<{ isSelected?: boolean; isFilled?: boolean; }>`
  width: 24px;
  height: 24px;
  background: ${({ theme, isFilled }) =>
    isFilled ? hexToRgba(theme.p300, 15) : "transparent"};
  cursor: pointer;
  margin: 0 4px;
  display: flex;
  align-items: center;
  ...
  border-radius: 2px;
  transition: ${({ isFilled }) => (isFilled ? "all 300ms" : "none")};
...
const Arrow = styled(ArrowDown)<{ rotate: number; }>`
  width: 10px;
  height: 10px;
...
const DoubleArrow = styled(DoubleArrowDown)<{ rotate: number; }>`
  width: 14px;
  height: 14px;
  transform: rotate(${({ rotate }) => `${rotate}deg`});
  * {
